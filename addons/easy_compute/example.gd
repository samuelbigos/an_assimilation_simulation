...
func texture_fill():
	...
	var fill_color = PackedFloat32Array([1.0, 1.0, 0.0, 1.0]) # RGBA
	compute.register_storage_buffer("fill_color", 1, 0, fill_color.to_byte_array())
	
	var noise_tex_image = noiseTex.get_image()
	var noise_image = Image.create(64, 64, false, Image.FORMAT_RGBA8)
	for x in range(0, 64):
		for y in range(0, 64):
			var col = noise_tex_image.get_pixel(x, y);
			noise_image.set_pixel(x, y, col)
			
	compute.register_texture("noise_texture", 2, 64, 64, noise_image.get_data(), RenderingDevice.DATA_FORMAT_R8G8B8A8_UNORM)
